// Глобальные переменные (могут быть объявлены вне слушателя)
let mediaRecorder;
let recordedChunks = [];
...
// Ждем полной загрузки HTML документа перед выполнением остального кода
document.addEventListener('DOMContentLoaded', function() {
    // Элементы DOM - Теперь получаем их после загрузки DOM
    const videoSourceSelect = document.getElementById('videoSource');
    const resolutionSelect = document.getElementById('resolution');
    const fpsSelect = document.getElementById('fps');
    const audioSourceSelect = document.getElementById('audioSource');
    const volumeSlider = document.getElementById('volume'); // Теперь этот элемент должен быть доступен
    ...
    // Слушатели событий - Привязываем их после получения элементов
    ...
    // !!! ПЕРЕСМОТРЕННАЯ ВЕРСИЯ startRecording !!!
    ...
    function updateUI() {
        ...
        // или если запись не идет и не на паузе (для начала записи)
         ...
         // Обновление текста статуса в зависимости от состояния, если он не pending или error
...
}); // Конец слушателя DOMContentLoaded