...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function toggleRecording() {
        if (!isRecording) {
             ...
             // Начинаем процесс запроса медиапотока
    ...
    // Функция для запроса медиапотока у пользователя
    async function requestMediaStream() {
         ...
         try {
             ...
             if (videoTracks.length === 0) {
              ...
              // Если поток успешно получен, проверяем таймер обратного отсчета
              if (countdownDuration > 0) {
                  startCountdown(screenStream, resolution, fps, videoSource, selectedRegion); // Передаем поток и другие параметры
              } else {
                  // Если отсчет выключен, сразу начинаем запись
                  actuallyStartRecording(screenStream, resolution, fps, videoSource, selectedRegion);
              }

         } catch (error) {
             console.error('Ошибка при запросе медиапотока:', error);
             // Улучшенные сообщения об ошибках
             let errorMessage = 'Ошибка при запросе медиапотока';
             if (error.name === 'NotAllowedError') {
                 ...
                 errorMessage = "Запрошенные параметры записи не поддерживаются.";
                  ...
                  errorMessage = "Произошла ошибка при попытке получить доступ к медиа устройствам. Возможно, другое приложение использует устройство."
             }
             else {
             ...
             statusDisplay.className = 'status error';
              // Сбрасываем UI в состояние готовности
              updateUI();
         }
    }
    ...
    async function startCountdown(stream, resolution, fps, videoSource, selectedRegion) {
        // Показываем таймер и скрываем основной интерфейс, который может мешать
        countdownTimerDisplay.style.display = 'block';
        // Добавляем стили для центрирования и видимости
        countdownTimerDisplay.style.position = 'fixed';
        countdownTimerDisplay.style.top = '50%';
        countdownTimerDisplay.style.left = '50%';
        countdownTimerDisplay.style.transform = 'translate(-50%, -50%)';
        countdownTimerDisplay.style.fontSize = '8em';
        countdownTimerDisplay.style.color = 'red';
        countdownTimerDisplay.style.zIndex = '100'; // Убедимся, что он поверх всего
        ...
        const countdownInterval = setInterval(() => {
         ...
         // Обработка отмены пользователем во время отсчета
         // Добавляем слушатель на событие "ended" на видеодорожке потока
        stream.getVideoTracks()[0].addEventListener('ended', () => {
             console.log("Поток завершен во время отсчета. Отмена записи.");
             clearInterval(countdownInterval); // Останавливаем отсчет
             countdownTimerDisplay.style.display = 'none'; // Скрываем таймер
             document.querySelector('.container').style.display = 'block'; // Показываем интерфейс
             statusDisplay.textContent = 'Запись отменена';
    ...
    // Новая функция для фактического начала записи после отсчета или сразу
    async function actuallyStartRecording(screenStream, resolution, fps, videoSource, selectedRegion) {
        try {
            // Скрываем таймер обратного отсчета (на случай, если он не был скрыт после отмены)
            countdownTimerDisplay.style.display = 'none';
            ...
            statusDisplay.className = 'status recording';
             ...
             // Проверяем успешность получения видеодорожки (уже проверено ранее)
             const videoTracks = screenStream.getVideoTracks();
             if (videoTracks.length === 0) {
                 throw new Error("Не удалось получить видеодорожку.");
            }
            ...
            // 2. Получаем аудиопоток с микрофона, если нужно (логика перенесена)
            let micStream = null;
            ...
            // 3. Создаем объединенный поток (логика перенесена)
            ...
            console.log("Запись успешно начата");
             ...
             // Показываем контейнер для визуализации кликов при старте записи, если включено
            if (showMouseClicks) {
    ...
    function updateUI() {
        ...
        // Отключаем кнопку настроек во время записи или обратного отсчета или запроса медиа
        btnSettings.disabled = isActivelyRecording || countdownTimerDisplay.style.display === 'block' || statusDisplay.className === 'status pending';

        // Скрываем индикатор уровня звука, если аудиоисточник не микрофон или идет обратный отсчет или запрос медиа
        if ((audioSourceSelect.value === 'mic' || audioSourceSelect.value === 'both') && countdownTimerDisplay.style.display !== 'block' && statusDisplay.className !== 'status pending') {
        ...
        // Обновляем видимость контейнера для визуализации кликов
    ...
    function showHelp() {
        alert("Справка по Веб-рекордеру экрана\n\n" +
            ...
            "4. Нажмите 'Начать запись' или Alt+R. Появится диалог выбора источника от браузера. После выбора, если включен таймер обратного отсчета в настройках, начнется отсчет перед записью.\n" +
